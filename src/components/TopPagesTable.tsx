        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-800">
              ...
              {(["views","uniqueVisitors","bounceRate"] as SortKey[]).map(col => (
                <th key={col} onClick={() => handleSort(col)}
                  ...
                  <div className="flex items-center justify-end gap-1">
                    {col === "views" ? "Views" : col === "uniqueVisitors" ? "Unique" : "Bounce %"}
                    <SortIcon col={col} />
                  </div>
                </th>
              ))}
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Avg Time</th>
            ...
            {loading ? (
              [...Array(5)].map((_, i) => (
                <tr key={i} className="border-b border-gray-800/50">
                  {[...Array(6)].map((_, j) => (
                    <td key={j} className="px-4 py-4"><div className="h-3 bg-gray-800 rounded animate-pulse" /></td>
                  ))}
                </tr>
              ))
            ) : sorted.map((row, i) => (
              <tr key={row.id} className={clsx("border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors", i === sorted.length-1 && "border-b-0")}>
                ...
                <td className="px-4 py-3.5 text-right text-gray-300 font-medium">{row.views.toLocaleString()}</td>
                <td className="px-4 py-3.5 text-right text-gray-400">{row.uniqueVisitors.toLocaleString()}</td>
                ...
                <td className="px-4 py-3.5 text-center">
                  <span className={clsx("px-2 py-0.5 rounded-full text-xs font-medium border", statusCfg[row.status].cls)}>
