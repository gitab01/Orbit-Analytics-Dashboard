...
const SEV_CFG: Record<AlertSeverity, { icon: React.ElementType; color: string; bg: string; border: string }> = {
  ...
  info:     { icon: Zap,            color: "text-blue-400",   bg: "bg-blue-500/10",   border: "border-blue-500/30"   },
  resolved: { icon: CheckCircle,    color: "text-brand-400",  bg: "bg-brand-500/10",  border: "border-brand-500/20"  },
};
...
export default function AlertsTab() {
  ...
  const STATS = [
    { label:"Active Alerts", value: active,          icon: Bell,         color:"text-red-400",    bg:"bg-red-500/10"    },
    { label:"Resolved Today",value: resolved,         icon: CheckCircle,  color:"text-brand-400",  bg:"bg-brand-500/10"  },
    { label:"Snoozed",       value: snoozed,          icon: Activity,     color:"text-amber-400",  bg:"bg-amber-500/10"  },
    { label:"Alert Rules",   value: rules.length,     icon: TrendingDown, color:"text-blue-400",   bg:"bg-blue-500/10"   },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-gray-900 border border-gray-800 rounded-xl">
              ...
              {(["all","critical","warning","info","snoozed"] as FilterKey[]).map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={clsx(
                    ...
                    filter === f ? "bg-brand-500/15 text-brand-400 border-brand-500/30" : "text-gray-500 border-gray-700 hover:text-gray-300"
                  )}>{f}</button>
              ))}
              <button onClick={fetchAll} className="p-1.5 rounded-lg text-gray-500 hover:text-gray-300 hover:bg-gray-800">
          ...
          <div className="divide-y divide-gray-800/60">
            ...
            {visible.map(alert => {
              ...
              const Icon = cfg.icon;
              return (
                <div key={alert.id} className={clsx("flex gap-4 p-5 hover:bg-gray-800/30 transition-colors", alert.status === "resolved" && "opacity-50")}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        {alert.status === "active" && <span className={clsx("w-1.5 h-1.5 rounded-full animate-pulse", alert.severity === "critical" ? "bg-red-400" : "bg-amber-400")} />}
                      </div>
                  ...
                  {alert.status === "active" && (
                    <div className="flex flex-col gap-1.5 flex-shrink-0">
                      ...
                      <button onClick={() => patchAlert(alert.id, "snoozed")} title="Snooze"
                        className="p-1.5 rounded-lg hover:bg-amber-500/20 text-gray-600 hover:text-amber-400 transition-colors">
                        <Activity size={13} />
                      </button>
                    </div>
                  )}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          {showForm && (
            <div className="mb-4 p-3 bg-gray-800/60 rounded-xl border border-gray-700 space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowForm(false)} className="flex-1 py-2 bg-gray-700 text-gray-300 text-xs rounded-lg hover:bg-gray-600 transition-colors">Cancel</button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            {rules.map(rule => (
              <div key={rule.id} className="p-3 bg-gray-800/50 rounded-xl border border-gray-700/50">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-medium text-white truncate pr-2">{rule.name}</span>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => toggleRule(rule.id, rule.enabled)}
                      className={clsx("relative w-9 h-5 rounded-full transition-colors", rule.enabled ? "bg-brand-500" : "bg-gray-700")}
                ...
                <p className="text-xs text-gray-500">Trigger: <span className="text-gray-400">{rule.trigger}</span></p>
                <p className="text-xs text-gray-500 mt-0.5">Channel: <span className="text-gray-400">{rule.channel}</span></p>
              </div>
            ))}
          </div>
        </div>
