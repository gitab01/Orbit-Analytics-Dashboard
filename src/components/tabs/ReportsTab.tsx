...
import { Download, FileText, TrendingUp, Users, Activity, Calendar, BarChart2, Plus, Trash2, RefreshCw } from "lucide-react";
        ...
        {TYPES.map(({ id, label, icon:Icon, color, desc }) => (
          <button key={id} onClick={() => setActiveType(id)}
            className={clsx("text-left p-3 sm:p-4 rounded-xl border transition-all",
              activeType === id
                ? "bg-white dark:bg-gray-800 border-brand-300 dark:border-brand-500/30 shadow-sm"
          ...
          {data && (
            <div className="grid grid-cols-3 gap-3 mt-5">
              {data.kpi.slice(0,3).map(m => (
                <div key={m.id} className="bg-gray-50 dark:bg-gray-800/60 rounded-lg p-3 border border-gray-100 dark:border-gray-700/50">
                  <p className="text-xs text-gray-500 mb-1">{m.label}</p>
                  <p className="text-sm font-bold text-gray-900 dark:text-white">{m.value}</p>
                  <p className="text-xs text-brand-600 dark:text-brand-400 mt-0.5">{m.change}</p>
                </div>
              ))}
            </div>
          )}
                ...
                <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                  <td className="px-4 sm:px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-900 dark:text-white font-medium text-xs sm:text-sm">{r.name}</span>
                    </div>
                  </td>
                  <td className="hidden sm:table-cell px-4 sm:px-5 py-3.5 text-gray-500 capitalize text-xs sm:text-sm">{r.type}</td>
                  <td className="hidden md:table-cell px-4 sm:px-5 py-3.5 text-gray-500 text-xs sm:text-sm">{r.freq}</td>
                  <td className="hidden md:table-cell px-4 sm:px-5 py-3.5 text-gray-500 text-xs sm:text-sm">{r.last}</td>
                  <td className="px-4 sm:px-5 py-3.5">
                  ...
                  <td className="px-4 sm:px-5 py-3.5 text-right">
                    <button onClick={() => deleteReport(r.id)} className="text-gray-400 hover:text-red-500 transition-colors p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10">
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>
