...
const renderActiveShape = (props: any) => {
  ...
  return (
    <g>
      <text x={cx} y={cy - 10} textAnchor="middle" fill="currentColor" fontSize={20} fontWeight={700}
        className="fill-gray-900 dark:fill-white">{value}%</text>
      ...
      <div className="mt-2 space-y-1.5">
        {traffic.map(e => (
          <div key={e.name} className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-gray-900 dark:text-white">{e.value}%</span>
          </div>
        ))}
      </div>
    </div>
