    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 shadow-xl">
      ...
      {payload.map((e: any) => (
        <div key={e.dataKey} className="flex items-center gap-2 text-sm">
          ...
          <span className="text-gray-500 dark:text-gray-400 capitalize">{e.dataKey}:</span>
          <span className="text-gray-900 dark:text-white font-medium">{fmt(e.dataKey, e.value)}</span>
        </div>
      ))}
    </div>
  );
            ...
            <button key={m.key} onClick={() => toggle(m.key)}
              className={clsx(
                ...
                active.includes(m.key)
                  ...
                  : "border-gray-200 dark:border-gray-800 text-gray-400 dark:text-gray-600 hover:text-gray-600 dark:hover:text-gray-400"
              )}>
              <span className="w-2 h-2 rounded-full"
                style={{ background: active.includes(m.key) ? m.color : "#9ca3af" }} />
              {m.label}
            </button>
          ))}
        </div>
        ...
        <AreaChart data={data} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
          ...
          {METRICS.map(m => active.includes(m.key) ? (
            <Area key={m.key} type="monotone" dataKey={m.key}
              ...
              activeDot={{ r: 4, fill: m.color, stroke: "#111827", strokeWidth: 2 }} />
          ) : null)}
        </AreaChart>
      </ResponsiveContainer>
    </div>
