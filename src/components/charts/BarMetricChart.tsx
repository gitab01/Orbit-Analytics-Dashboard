...
export default function BarMetricChart({ data }: BarMetricChartProps) {
  // Show last 14 days
  const sliced = data.slice(-14);
  const max = Math.max(...sliced.map((d) => d.conversions));
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={sliced} barSize={16} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
          ...
          <Bar dataKey="conversions" radius={[4, 4, 0, 0]}>
            {sliced.map((entry, index) => (
              <Cell
                key={`cell-${index}`}
                fill={entry.conversions === max ? "#15b382" : "#1e3a2f"}
              />
            ))}
          </Bar>
        </BarChart>
